...
# Стикер-пак и ID чата для автоматической рассылки
STICKER_SET = "POCX3"
# Замените на реальный ID чата/канала (например, -1001234567890)
STICKER_CHAT_ID = -1001234567890   # ❗️ ОБЯЗАТЕЛЬНО ИЗМЕНИТЕ
STICKER_IDS = [f"{STICKER_SET}_{i}" for i in range(1, 13)]  # если стикеров 12
...
class MinesGame:
    ...
    def reveal(self, user_id: int, cell: int) -> dict:
        ...
        return {
            "result": "diamond",
            "multiplier": multi,
            "win_amount": int(g["bet"] * multi),
            "remaining": self.GRID_SIZE - g["mines_count"] - g["diamonds"]
        }
...
# ==================== REPLY-КЛАВИАТУРЫ (ПРИКРЕПЛЁННЫЕ) ====================
def main_reply_keyboard():
    kb = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="ℹ️ Профиль"), KeyboardButton(text="👑 Админ-панель")]
        ],
        resize_keyboard=True
    )
    # Если пользователь не админ, убираем админ-кнопку (но клавиатура статична, проще оставить, реагировать только если админ)
    return kb
...
def games_reply_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🗼 Башня"), KeyboardButton(text="💣 Mines")],
            [KeyboardButton(text="🎡 Roulette"), KeyboardButton(text="🎰 Слоты")],
...
def top_reply_keyboard():
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="🏆 За всё время"), KeyboardButton(text="📅 За неделю")],
            [KeyboardButton(text="📆 За день"), KeyboardButton(text="👥 Топ рефералов")],
            [KeyboardButton(text="🔙 Главное меню")]
        ],
        resize_keyboard=True
    )
...
# ==================== INLINE-КЛАВИАТУРЫ ДЛЯ ИГР (ВНУТРИ СООБЩЕНИЯ) ====================
def dice_menu_inline():
...
def tower_game_inline(step: int, taps_left: int):
    kb = InlineKeyboardMarkup(inline_row_width=2)
    for i in range(4):
...
def mines_grid_inline(game: dict):
...
def hilo_controls_inline(can_cashout: bool = False):
...
def donate_inline():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔙 Назад", callback_data="finance_back")]
    ])


# ==================== ФОНОВАЯ ЗАДАЧА — СТИКЕРЫ КАЖДЫЕ 10 МИНУТ ====================
async def sticker_sender():
    """Отправляет случайный стикер из пака POCX3 в указанный чат каждые 10 минут."""
    if STICKER_CHAT_ID == -1001234567890:
        # Если пользователь не изменил ID, просто выходим, чтобы не спамить в пустоту
        return
    while True:
        await asyncio.sleep(600)  # 10 минут
        sticker = random.choice(STICKER_IDS)
        try:
            await bot.send_sticker(STICKER_CHAT_ID, sticker)
        except Exception as e:
            print(f"Ошибка отправки стикера: {e}")


# ==================== ОБРАБОТЧИКИ ТЕКСТОВЫХ КОМАНД (ИГРЫ В ЧАТАХ) ====================
@dp.message(lambda msg: msg.text and msg.text.lower().startswith("мины "))
async def text_mines_command(message: Message):
    args = message.text.split()
    if len(args) != 2 or not args[1].isdigit():
        await message.answer("❌ Используйте: `мины 1000`")
        return
    bet = int(args[1])
    if bet != 1000:
        await message.answer("❌ В Mines фиксированная ставка 1000 POCX")
        return
    # Переиспользуем логику mines_bet
    await process_mines_bet(message, bet)


async def process_mines_bet(message: Message, bet: int):
    ...
    if not remove_pocx(message.from_user.id, bet):
        await message.answer("❌ Недостаточно POCX!", reply_markup=main_reply_keyboard())
        return
    game = mines_games.start(message.from_user.id, bet, 5)
    await message.answer(
        f"💣 <b>Mines</b>\n\n💣 Мин: 5 | 💵 {bet:,} POCX\n💎 Открыто: 0 | 📈 1.00x\n\nВыбери клетки (поле 5x5):",
        reply_markup=mines_grid_inline(game),
        parse_mode=ParseMode.HTML
    )


@dp.message(lambda msg: msg.text and msg.text.lower().startswith("башня "))
async def text_tower_command(message: Message, state: FSMContext):
    args = message.text.split()
    if len(args) != 3 or not args[1].isdigit() or args[2] not in ['1','2','3']:
        await message.answer("❌ Используйте: `башня [ставка] [1-3]`\nПример: `башня 5000 1`")
        return
    bet = int(args[1])
    bombs = int(args[2])
    ...
    await message.answer(
        f"🗼 <b>Игра Башня</b>\n\n💣 Бомб: {bombs}\n💰 Ставка: {bet:,} POCX\n🎯 Осталось тапов: 9\n\nВыбери ячейку (1-4):",
        reply_markup=tower_game_inline(0, 9),
        parse_mode=ParseMode.HTML
    )


@dp.message(lambda msg: msg.text and msg.text.lower().startswith("кубик "))
async def text_dice_command(message: Message, state: FSMContext):
    args = message.text.lower().split()
    if len(args) < 3:
        await message.answer("❌ Используйте: `кубик [сумма] [чёт/нечет/число]`\nПример: `кубик 1000 чёт`")
        return
    try:
        bet = int(args[1])
    except:
    ...
    if not remove_pocx(message.from_user.id, bet):
        await message.answer("❌ Недостаточно POCX!", reply_markup=main_reply_keyboard())
        return
    result = DiceGame.roll(50, 49.5)  # временный заглушка — дальше нужно определить target/chance по выбору
    # Для простоты сделаем выбор: если args[2] == "чёт" или "нечет" — фиксированный шанс 49.5%
    # Но так как это текстовая команда, проще свести к выбору из двух вариантов.
    # Чтобы не усложнять, предлагаем использовать команду только для чёт/нечет с множителем 2x.
    choice = args[2]
    if choice == "чёт":
        target = 50
        chance = 49.5
        ...
        target = 16  # 16% шанс (1-6) даст ~6x, но для простоты оставим заглушку
        chance = 16.0
    else:
        await message.answer("❌ Неверный формат. Используйте 'чёт', 'нечет' или число 1-6")
        return
    roll = DiceGame.roll(target, chance)
    win = int(bet * roll["multiplier"]) if roll["is_win"] else 0
    if roll["is_win"]:
        add_pocx(message.from_user.id, win)
    record_game(message.from_user.id, "dice", bet, roll["multiplier"] if roll["is_win"] else 0, win, "win" if roll["is_win"] else "loss", {"roll": roll["roll"], "target": target})
    user = get_user(message.from_user.id)
    if roll["is_win"]:
        msg = (f"🎲 <b>Dice</b>\n\nВыпало: {roll['roll']} | Цель: <{target}\n✅ ВЫИГРЫШ!\n"
               f"💰 Ставка: {bet:,} POCX\n🎉 Выигрыш: {win:,} POCX ({roll['multiplier']}x)\n\n"
               f"💰 Баланс: {user['balance']:,} POCX")
    else:
        msg = (f"🎲 <b>Dice</b>\n\nВыпало: {roll['roll']} | Цель: <{target}\n❌ ПРОИГРЫШ\n"
               f"💰 Ставка: {bet:,} POCX\n😢 Вы проиграли\n\n"
               f"💰 Баланс: {user['balance']:,} POCX")
    await message.answer(msg, reply_markup=main_reply_keyboard(), parse_mode=ParseMode.HTML)


@dp.message(lambda msg: msg.text and msg.text.lower().startswith("краш "))
async def text_crash_command(message: Message, state: FSMContext):
    args = message.text.split()
    if len(args) != 2 or not args[1].isdigit():
        await message.answer("❌ Используйте: `краш 1000`")
        return
    bet = int(args[1])
    if bet < MIN_BET or bet > MAX_BET:
        await message.answer(f"❌ Ставка от {MIN_BET} до {MAX_BET} POCX")
    ...
    msg = await message.answer(
        f"🚀 <b>ВЗЛЁТ!</b>\n\n⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜ 1.00x\n\n💵 Ставка: {bet:,} POCX\n💰 Потенциал: {bet:,} POCX",
        reply_markup=crash_control_inline(),
        parse_mode=ParseMode.HTML
    )
    asyncio.create_task(crash_animation(message.from_user.id, msg, bet, game["crash_point"], state))


# ==================== ОСНОВНЫЕ ОБРАБОТЧИКИ МЕНЮ (REPLY) ====================
@dp.message(F.text == "🎮 Игры")
async def games_menu(message: Message):
    await message.answer("🎮 <b>Выбери игру</b>", reply_markup=games_reply_keyboard(), parse_mode=ParseMode.HTML)

@dp.message(F.text == "💰 Финансы")
async def finance_menu(message: Message):
    await message.answer("💰 <b>Финансы</b>", reply_markup=finance_reply_keyboard(), parse_mode=ParseMode.HTML)

@dp.message(F.text == "🏆 Топы")
async def top_menu(message: Message):
    await message.answer("🏆 <b>Топы</b>\n\nВыбери категорию:", reply_markup=top_reply_keyboard(), parse_mode=ParseMode.HTML)

@dp.message(F.text == "🎁 Бонусы")
async def bonuses_menu(message: Message):
    await message.answer("🎁 <b>Бонусы</b>\n\nЕжедневный бонус: 100-2000 POCX\nПриветственный: 1000 POCX", reply_markup=bonuses_reply_keyboard(), parse_mode=ParseMode.HTML)

@dp.message(F.text == "👥 Рефералы")
async def referrals_menu(message: Message):
    user = get_user(message.from_user.id)
    if not user:
        await message.answer("❌ Ошибка", reply_markup=main_reply_keyboard())
        return
    ...
    await message.answer(
        f"👥 <b>Рефералы</b>\n\n🔗 Ваша ссылка:\n<code>{link}</code>\n\n👤 Рефералов: {count}\n💰 Заработано: {earned:,} POCX\n\n🎁 За каждого друга: +{REF_REWARD:,} POCX",
...
async def admin_panel_entry(message: Message):
    if message.from_user.id not in ADMIN_IDS:
        ...
        return
    await message.answer("👑 <b>Админ-панель</b>", reply_markup=admin_reply_keyboard(), parse_mode=ParseMode.HTML)

@dp.message(F.text == "🔙 Главное меню")
async def back_to_main(message: Message):
    await message.answer("🏠 <b>Главное меню</b>", reply_markup=main_reply_keyboard(), parse_mode=ParseMode.HTML)

@dp.message(F.text == "🔙 Назад")
async def back_to_games(message: Message):
    await message.answer("🎮 <b>Выбери игру</b>", reply_markup=games_reply_keyboard(), parse_mode=ParseMode.HTML)

@dp.callback_query(F.data == "games_back")
async def back_to_games_callback(callback: CallbackQuery):
    await callback.message.edit_text("🎮 <b>Выбери игру</b>", reply_markup=games_reply_keyboard(), parse_mode=ParseMode.HTML)
    await callback.answer()

@dp.callback_query(F.data == "finance_back")
async def finance_back_callback(callback: CallbackQuery):
    await callback.message.edit_text("💰 <b>Финансы</b>", reply_markup=finance_reply_keyboard(), parse_mode=ParseMode.HTML)
    await callback.answer()


# ==================== ОБРАБОТЧИКИ ИГР (INLINE) ====================
# Dice
@dp.message(F.text == "🎲 Dice")
async def dice_inline_start(callback: Message):
    await callback.answer("🎲 <b>Dice</b>\n\nВыбери шанс:", reply_markup=dice_menu_inline(), parse_mode=ParseMode.HTML)

@dp.callback_query(F.data.startswith("dice_"))
async def dice_choice_callback(callback: CallbackQuery, state: FSMContext):
...
async def dice_bet_process(message: Message, state: FSMContext):
    try:
        bet = int(message.text.replace(",", ""))
        if bet < MIN_BET or bet > MAX_BET:
            raise ValueError
    except:
        await message.answer(f"❌ От {MIN_BET:,} до {MAX_BET:,} POCX!", reply_markup=games_reply_keyboard())
        return
    if not check_cooldown(message.from_user.id):
        await message.answer("⏳ Подожди 5 секунд!", reply_markup=games_reply_keyboard())
        return
    if not remove_pocx(message.from_user.id, bet):
        await message.answer("❌ Недостаточно POCX!", reply_markup=main_reply_keyboard())
        await state.clear()
    ...
    if result["is_win"]:
        ...
        msg = (f"🎲 <b>Dice</b>\n\nВыпало: {result['roll']} | Цель: <{tgt}\n❌ ПРОИГРЫШ\n"
               f"💰 Ставка: {bet:,} POCX\n😢 Вы проиграли\n\n"
               f"💰 Баланс: {user['balance']:,} POCX")
    await message.answer(msg, reply_markup=games_reply_keyboard(), parse_mode=ParseMode.HTML)
    await state.clear()
...
async def crash_bet_process(message: Message, state: FSMContext):
    try:
        bet = int(message.text.replace(",", ""))
        if bet < MIN_BET or bet > MAX_BET:
            raise ValueError
    ...
    if not check_cooldown(message.from_user.id):
        ...
        return
    if not remove_pocx(message.from_user.id, bet):
        await message.answer("❌ Недостаточно POCX!", reply_markup=games_reply_keyboard())
        await state.clear()
        return
    game = CrashGame().start(message.from_user.id, bet)
    crash_games[message.from_user.id] = game
...
async def crash_cashout_callback(callback: CallbackQuery):
...
async def tower_start_inline(message: Message):
    await message.answer("🗼 <b>Башня</b>\n\nВыбери количество бомб:", reply_markup=tower_bombs_menu_inline(), parse_mode=ParseMode.HTML)

@dp.callback_query(F.data.startswith("tower_bombs_"))
async def tower_bombs_callback(callback: CallbackQuery, state: FSMContext):
...
async def tower_bet_process(message: Message, state: FSMContext):
    try:
        bet = int(message.text.replace(",", ""))
        if bet < MIN_BET or bet > MAX_BET:
            raise ValueError
...
async def tower_tap_callback(callback: CallbackQuery):
    uid = callback.from_user.id
    ...
    await callback.message.edit_text(
        ...
        reply_markup=tower_game_inline(step, taps_left),
        parse_mode=ParseMode.HTML
    )
    await callback.answer()
...
async def mines_start_inline(message: Message, state: FSMContext):
    await state.set_state(BetState.waiting_amount)
    await message.answer("💣 <b>Mines</b>\n\n💵 Введите ставку (фиксированная 1000 POCX):", parse_mode=ParseMode.HTML)

@dp.message(BetState.waiting_amount)
async def mines_bet_inline(message: Message, state: FSMContext):
    try:
        bet = int(message.text.replace(",", ""))
        if bet != 1000:
            raise ValueError
    except:
        await message.answer("❌ Ставка должна быть 1000 POCX!", reply_markup=games_reply_keyboard())
        await state.clear()
        return
    await process_mines_bet(message, bet)
    await state.clear()
...
async def mines_reveal_callback(callback: CallbackQuery):
    uid = callback.from_user.id
    ...
    if result["result"] == "bomb":
        ...
        await callback.message.edit_text(
            f"💣 <b>Mines</b>\n\n💣 5 | 💵 {game['bet']:,} POCX\n💎 {game['diamonds']} | 📈 {result['multiplier']}x | 💰 {result['win_amount']:,} POCX",
            reply_markup=mines_grid_inline(game),
            parse_mode=ParseMode.HTML
        )
    await callback.answer()
...
async def mines_cashout_callback(callback: CallbackQuery):
    uid = callback.from_user.id
    ...
    add_pocx(uid, win)
    await callback.message.edit_text(
        f"🎉 <b>ВЫИГРЫШ!</b>\n\n💎 Алмазов: {game.get('diamonds', 0)}\n📈 {game.get('current_multiplier', 1)}x\n💰 Выигрыш: {win:,} POCX",
        reply_markup=games_reply_keyboard(),
        parse_mode=ParseMode.HTML
    )
    mines_games.active.pop(uid, None)
...
async def roulette_start_inline(message: Message, state: FSMContext):
    await state.set_state(RouletteState.choice)
    await message.answer("🎡 <b>Рулетка</b>\n\nВыбери тип ставки:", reply_markup=roulette_menu_inline(), parse_mode=ParseMode.HTML)

@dp.callback_query(F.data.startswith("rl_"), RouletteState.choice)
async def roulette_choice_callback(callback: CallbackQuery, state: FSMContext):
...
async def roulette_bet_process(message: Message, state: FSMContext):
    try:
        bet = int(message.text.replace(",", ""))
        if bet < MIN_BET or bet > MAX_BET:
            raise ValueError
    ...
    user = get_user(message.from_user.id)
    win_line = f"💰 Выигрыш: {result['win_amount']:,} POCX ({result['multiplier']}x)" if result["is_win"] else "😢 Вы проиграли"
    await spin_msg.edit_text(
        f"🎡 <b>Рулетка</b>\n\n{result['color']} <b>{number}</b>\nСтавка: {labels[bet_type]}\n\n{'🎉 ВЫИГРЫШ!' if result['is_win'] else '❌ ПРОИГРЫШ'}\n{win_line}\n\n💰 Баланс: {user['balance']:,} POCX",
        reply_markup=games_reply_keyboard(),
        parse_mode=ParseMode.HTML
    )
    await state.clear()
...
async def slots_start_inline(message: Message, state: FSMContext):
...
async def slots_bet_process(message: Message, state: FSMContext):
    try:
        bet = int(message.text.replace(",", ""))
        if bet < MIN_BET or bet > MAX_BET:
            raise ValueError
    ...
    user = get_user(message.from_user.id)
    win_line = f"💰 Выигрыш: {win_amount:,} POCX ({multiplier}x)" if win_amount > 0 else "😢 Вы проиграли"
    await spin_msg.edit_text(
        f"🎰 <b>Слоты</b>\n\n[ {' | '.join(reels)} ]\n\n{'🎉 ВЫИГРЫШ!' if win_amount > 0 else '❌ ПРОИГРЫШ'}\n{win_line}\n\n💰 Баланс: {user['balance']:,} POCX",
        reply_markup=games_reply_keyboard(),
        parse_mode=ParseMode.HTML
    )
    await state.clear()
...
async def coin_start_inline(message: Message, state: FSMContext):
    await state.set_state(CoinState.choice)
    await message.answer("🪙 <b>Coin Flip</b>\n\nВыбери сторону:", reply_markup=coin_menu_inline(), parse_mode=ParseMode.HTML)

@dp.callback_query(F.data.startswith("coin_"), CoinState.choice)
async def coin_choice_callback(callback: CallbackQuery, state: FSMContext):
...
async def coin_bet_process(message: Message, state: FSMContext):
    try:
        bet = int(message.text.replace(",", ""))
        if bet < MIN_BET or bet > MAX_BET:
            raise ValueError
    ...
    win_line = f"💰 Выигрыш: {win:,} POCX (2x)" if result["is_win"] else "😢 Вы проиграли"
    await flip_msg.edit_text(
        f"🪙 <b>Coin Flip</b>\n\n{result['emoji']} Выпало: <b>{res_label}</b>\n\n{'🎉 ВЫИГРЫШ!' if result['is_win'] else '❌ ПРОИГРЫШ'}\n{win_line}\n\n💰 Баланс: {user['balance']:,} POCX",
        reply_markup=games_reply_keyboard(),
        parse_mode=ParseMode.HTML
    )
    await state.clear()
...
async def hilo_start_inline(message: Message, state: FSMContext):
...
async def hilo_bet_process(message: Message, state: FSMContext):
    try:
        bet = int(message.text.replace(",", ""))
        if bet < MIN_BET or bet > MAX_BET:
            raise ValueError
    ...
    await message.answer(
        f"🃏 <b>Hi-Lo - Раунд 1</b>\n\nОткрытая карта: <b>{game['current_card']}</b>\nМножитель: 1.00x\n\nСледующая карта будет...",
        reply_markup=hilo_controls_inline(False),
        parse_mode=ParseMode.HTML
    )

@dp.callback_query(F.data.in_({"hilo_higher", "hilo_lower", "hilo_same"}), HiLoState.active)
async def hilo_guess_callback(callback: CallbackQuery, state: FSMContext):
    ...
    if result["result"] == "win":
        ...
        await callback.message.edit_text(
            f"🃏 <b>Hi-Lo - Раунд {result['round']}</b>\n\n✅ Угадали!\nНовая карта: <b>{result['card']}</b>\nМножитель: <b>{result['multiplier']}x</b>\n💰 Можно забрать: <b>{int(game['bet'] * result['multiplier']):,} POCX</b>\n\nПродолжаем?",
            reply_markup=hilo_controls_inline(True),
            parse_mode=ParseMode.HTML
        )
    else:
        ...
        user = get_user(uid)
        await callback.message.edit_text(
            f"🃏 <b>Hi-Lo</b>\n\n❌ Не угадали!\nКарта была: <b>{result['card']}</b>\n\n💸 Проигрыш: {game.get('bet', 0):,} POCX\n💰 Баланс: {user['balance']:,} POCX",
            reply_markup=games_reply_keyboard(),
            parse_mode=ParseMode.HTML
        )
        await state.clear()
...
async def hilo_cashout_callback(callback: CallbackQuery, state: FSMContext):
    uid = callback.from_user.id
    ...
    user = get_user(uid)
    await callback.message.edit_text(
        f"🃏 <b>Hi-Lo - ВЫИГРЫШ!</b>\n\n📈 Множитель: <b>{multi}x</b>\n💵 Ставка: {game.get('bet', 0):,} POCX\n💰 <b>Выигрыш: {win:,} POCX</b>\n\n💰 Баланс: {user['balance']:,} POCX",
        reply_markup=games_reply_keyboard(),
        parse_mode=ParseMode.HTML
    )
    await state.clear()
...
async def daily_bonus(message: Message):
    ...
    row = cursor.fetchone()
    if not row:
        await message.answer("❌ Ошибка", reply_markup=bonuses_reply_keyboard())
        return
    last = row[0]
    streak = row[1] or 0
    if last:
    ...
    conn.commit()
    await message.answer(
        f"🎁 <b>Ежедневный бонус!</b>\n\n💰 +{bonus:,} POCX\n🔥 Серия дней: {streak + 1}\n📈 Бонус увеличен на {min(streak * 5, 50)}%!",
...
async def promo_use_process(message: Message, state: FSMContext):
...
async def top_all_time(message: Message):
    ...
    await message.answer("\n".join(lines), reply_markup=top_reply_keyboard(), parse_mode=ParseMode.HTML)
...
async def admin_stats(message: Message):
    ...
    await message.answer(
        f"📊 <b>Статистика казино</b>\n\n👥 Пользователей: {users}\n💰 Общий баланс: {total_balance:,} POCX\n📊 Общий вейджер: {total_wagered:,} POCX\n🏆 Выиграно всего: {total_won:,} POCX\n💸 Проиграно всего: {total_lost:,} POCX\n📈 Прибыль казино: {total_lost - total_won:,} POCX",
        reply_markup=admin_reply_keyboard(),
        parse_mode=ParseMode.HTML
    )

@dp.message(F.text == "➕ Выдать POCX")
async def admin_give(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("❌ Нет доступа!", reply_markup=main_reply_keyboard())
        return
    await state.set_state(BetState.waiting_amount)
...
async def admin_promo_create_entry(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("❌ Нет доступа!", reply_markup=main_reply_keyboard())
        return
    await state.set_state(PromoState.code)
...
@dp.message(PromoState.code)
async def admin_promo_create_process(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("❌ Нет доступа!", reply_markup=main_reply_keyboard())
        await state.clear()
        return
    ...
    conn.commit()
    await message.answer(f"✅ Промокод создан!\nКод: `{code}`\nСумма: {amount:,} POCX\nЛимит: {max_uses}", reply_markup=admin_reply_keyboard(), parse_mode=ParseMode.HTML)
    await state.clear()
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        f"🎰 <b>POCX Casino Bot</b>\n\n💰 Баланс: <b>{user['balance']:,} POCX</b>\n💎 VIP: {vip}\n\n🎮 Игры: Dice • Crash • Башня • Mines • Roulette • Слоты • Coin Flip • Hi-Lo\n\n⭐ 1 Star = 2500 POCX\n🔥 КД между играми: 5 секунд",
        reply_markup=main_reply_keyboard(),
        parse_mode=ParseMode.HTML
    )
...
async def main():
    ...
    # Запускаем фоновую задачу на отправку стикеров
    asyncio.create_task(sticker_sender())
