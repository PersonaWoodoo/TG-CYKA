...
async def cb_dice_choice(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"🎲 <b>Dice</b>\n\nШанс: {chance}%\n"
        f"💰 Ставка: x{round((100 / float(chance)) * (1 - HOUSE_EDGE), 2)}\n\n"
...
async def dice_bet(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"🎲 <b>Dice</b>\n\n"
        f"Выпало: {result['roll']} | Цель: <{tgt}\n"
        f"{emoji}\n"
        f"💰 Ставка: {bet:,} POCX\n"
        f"{f'🎉 Выигрыш: {win:,} POCX ({result["multiplier"]}x)' if result['is_win'] else '😢 Вы проиграли'}\n\n"
        f"💰 Баланс: {user['balance']:,} POCX",
        reply_markup=main_menu(),
        parse_mode=ParseMode.HTML
    )
...
async def crash_cashout(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"🎉 <b>ВЫИГРЫШ!</b>\n\n"
        f"📈 Забрано на <b>{cur:.2f}x</b>\n"
        f"💵 Ставка: {game['bet']:,} POCX\n"
        f"💰 Выигрыш: <b>{win:,} POCX</b>",
        reply_markup=main_menu(),
        parse_mode=ParseMode.HTML
    )
    crash_games.pop(uid)
