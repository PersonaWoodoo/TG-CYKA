...
# ==================== КОНФИГУРАЦИЯ ====================
TOKEN = "8134237711:AAH5M0JJULkrl0yyOdvfCtIswRPqRtt_3hg"  # НОВЫЙ ТОКЕН
...
async def cb_tower(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text(
        "🗼 <b>Башня</b>\n\n"
        "Правила:\n"
        "• 9 этажей, на каждом нужно выбрать безопасную ячейку\n"
        "• 1 бомба = +0.1x за тап\n"
        "• 2 бомбы = +0.25x за тап\n"
        "• 3 бомбы = +0.6x за тап\n\n"
...
async def tower_bombs_select(callback: CallbackQuery, state: FSMContext):
    bombs = int(callback.data.split("_")[2])
    await state.update_data(tower_bombs=bombs, tower_wins=0, tower_level=0)
    await callback.message.edit_text(
        f"🗼 <b>Башня</b>\n\n"
        f"💣 Бомб в ряду: {bombs}\n"
...
@dp.message(GameStates.tower_bet)
async def tower_bet_amount(message: Message, state: FSMContext):
    ...
    await state.update_data(tower_bet=bet, tower_level=0, tower_wins=0)
