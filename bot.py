...
STARS_TO_POCX = 2500  # 1 Star = 2500 POCX
...
def bet_amounts_menu(callback_prefix: str):
    kb = InlineKeyboardMarkup(inline_row_width=2)
    for amount in [100, 500, 1000, 5000, 10000, 50000, 100000, 500000]:
        kb.insert(InlineKeyboardButton(text=f"{amount:,}", callback_data=f"{callback_prefix}_{amount}"))
    kb.add(InlineKeyboardButton(text="🔙 Назад", callback_data="games"))
    return kb
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎰 Розыгрыш", callback_data="admin_raffle")],
...
@dp.callback_query(F.data.in_({"top_all", "top_week", "top_day"}))
async def cb_top_period(callback: CallbackQuery):
    ...
    medals = ["🥇", "🥈", "🥉"]
    lines = [f"🏆 <b>Топ {{'all':'за всё время','week':'за неделю','day':'за день'}[period]}</b>\n"]
...
async def cb_referrals(callback: CallbackQuery):
    ...
    link = f"https://t.me/{ (await bot.get_me()).username }?start=ref{user['referral_code']}"
...
async def cb_dice_choice(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"🎲 Шанс {chance}%\n\n💵 Введите ставку (от {MIN_BET:,} до {MAX_BET:,} POCX):", parse_mode=ParseMode.HTML)
...
async def cb_tower_bombs(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"🗼 Бомб: {bombs}\n\n💵 Введите ставку:", parse_mode=ParseMode.HTML)
...
async def cb_donate(callback: CallbackQuery):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⭐ 1 Star - 2500 POCX", callback_data="donate_1")],
...
async def main():
    ...
    print(f"🎮 Игры: Dice, Crash, Башня, Mines, Roulette, Слоты, CoinFlip, Hi-Lo")
